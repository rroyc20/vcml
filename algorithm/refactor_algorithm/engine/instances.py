...
def load_existing_instance(
    ...
    inst = {
        ...
        "use_sri_cuts": int(use_sri_cuts),
        "sri_cardinality": int(sri_cardinality),
        "enable_sri": int(enable_sri),
        "root_only_sri": int(root_only_sri),
        "max_sri_rounds": int(max_sri_rounds),
        "max_cuts_per_round": int(max_cuts_per_round),
        "max_cuts_per_day": int(max_cuts_per_day),
        "min_sri_violation": float(min_sri_violation),
        "enable_sri_similarity_filter": int(enable_sri_similarity_filter),
        "max_shared_edges_between_sri3": int(max_shared_edges_between_sri3),
...
def list_existing_instances(instance_dir: str | Path) -> List[Path]:
    base = _resolve_input_path(instance_dir)
