...
from refactor_algorithm.core.pricing.cut_pricing import build_cut_pricing_state, normalize_cut_pricing_mode
from refactor_algorithm.core.util.gurobi_snapshot import (
    snapshot_constr_pi_by_name,
    snapshot_var_values_by_name,
)
...
class BnBNode:
        ...
        for _ in range(config.max_cg_iterations_per_node):
            ...
            if not fractional:
                ...
                if _d_sol is not None and _rmp_u is not None and hasattr(
                    ...
                    try:
                        _av_u = snapshot_var_values_by_name(
                            self._get_gurobi_model(),
                            eps=1e-12,
                        )
    ...
    def solve_rmp(self) -> Tuple[float, Dict[str, Any]]:
        ...
        # RMP는 LP relaxation이어야 하므로 필요할 때만 연속 변수 설정을 강제한다.
        if not bool(getattr(self, "_lp_relaxation_ready", False)):
            changed_vtypes = False
            ...
            if changed_vtypes:
                model.update()
            self._lp_relaxation_ready = True
        ...
        if status == GRB.OPTIMAL:
            ...
            dual_values["constr_pi_by_name"] = snapshot_constr_pi_by_name(model)
    ...
    def solve_subproblem(self, dual_values: Dict[str, Any], config: BnBConfig) -> PricingResult:
        ...
        if use_lex_vehicle_dual:
            from refactor_algorithm.core.pricing.lex_vehicle_dual import adjust_vehicle_dual_with_lex
        ...
        if not use_transformed_graph and isinstance(inst_dict, dict):
            ...
            _discount_sc, discount_sp_path = self._get_sparse_discount_path_apsp_cached(
                inst_dict,
                depot,
        ...
        for day_idx, day_ctx in enumerate(days):
            ...
            if effective_pricing_method == "cpp_ng":
                ...
                try:
                    from refactor_algorithm.core.pricing.cpp_pricer import solve_day_cpp_ngroute
                    ...
                    cpp_cols = solve_day_cpp_ngroute(
                        ...
                        discount_sp_path=discount_sp_path,
            ...
            if effective_pricing_method == "cpp_dp":
                _tcc = time.perf_counter()
                try:
                    from refactor_algorithm.core.pricing.cpp_pricer import solve_day_cpp_dp
    ...
    def get_integer_solution_if_any(self) -> Optional[Any]:
        ...
        if _dis is not None and _rmp_o is not None and hasattr(
            ...
            try:
                armp_f = snapshot_var_values_by_name(model, eps=1e-12)
...
class BnBTree:
    ...
    def _seed_initial_upper_bound_if_available(self) -> None:
        ...
        try:
            from refactor_algorithm.core.master.compare_arc_vs_bnp import seed_bnb_tree_initial_ub
