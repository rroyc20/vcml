...
Usage:
  python -m refactor_algorithm.cli.inspect_bnp [options]

  # 기본: full instance(전체 원본). 서브샘플은 --full-instance 0
  python -m refactor_algorithm.cli.inspect_bnp --instance data/existing/yao/yao-westjordan-S.dat

  # 처음 50노드만 출력
  python -m refactor_algorithm.cli.inspect_bnp --instance data/existing/yao/yao-westjordan-S.dat --max-nodes 50

  # JSON Lines 파일로 저장
  python -m refactor_algorithm.cli.inspect_bnp --instance data/existing/yao/yao-westjordan-S.dat --out inspect_yao_s.jsonl
"""
...
from refactor_algorithm.app.inspection_output import (
    print_incumbent_discount_report as _print_incumbent_discount_report,
    print_node_summary as _print_node_summary,
    print_pricing_breakdown as _print_prc_s_breakdown,
    profile_sum as _prof_sum,
)
from refactor_algorithm.app.inspection_records import (
    CGIterRecord,
    NodeRecord,
    branch_candidate_metrics as _branch_candidate_metrics,
    branch_candidates_equal as _branch_candidates_equal,
    fmt_value as _fmt,
)
...
from refactor_algorithm.core.util.gurobi_snapshot import snapshot_var_values_by_name
...
def _inspect_print_sum_k_lambda_over_route(node: Any) -> None:
    ...
    from refactor_algorithm.core.util.initial_heuristic import canon_edge as _canon_edge
    ...
    def _accumulate(
        ...
        for vname in names:
            ...
            if ridx < 0 or ridx >= len(cols):
                continue
            xv = values_by_name.get(str(vname))
            if xv is None:
                continue
            col = cols[ridx]
            key = _rk(col)
            by_bucket[key] = by_bucket.get(key, 0.0) + float(xv)

    sums: Dict[Tuple[int, Tuple[Tuple[int, int], ...], Tuple[Tuple[int, int], ...]], float] = {}
    used_simple_sp = False
    values_by_name = snapshot_var_values_by_name(
        m,
        eps=None,
        prefixes=("lam_", "lambda_", "agg_lam_", "alm_"),
    )
    ...
    if route_cols is not None and by_tk is not None and n2i is not None:
        ...
        if sample is not None and isinstance(sample, tuple) and len(sample) == 2:
            for _names in by_tk.values():
                _accumulate(sums, _names, n2i, route_cols, values_by_name)
            used_simple_sp = True
    ...
    if not used_simple_sp:
        ...
        for _names in by_day.values():
            _accumulate(sums, _names, n2i_agg, agg_cols, values_by_name)
...
class InspectBnBNode(BnBNode):
    ...
    def solve_node(self, config: BnBConfig, incumbent_ub: float) -> NodeSolveResult:
        ...
        for _ in range(config.max_cg_iterations_per_node):
            ...
            phase1_active = art_sum_now > 1e-8
            if phase1_active:
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    parser.add_argument(
        "--use-transformed-pricing-graph",
        type=int,
        choices=[0, 1],
        default=1,
        help="1=sparse -> transformed pricing graph 사용, 0=원래 pricing graph 사용.",
    )
    parser.add_argument(
        "--quiet-nodes",
        action="store_true",
