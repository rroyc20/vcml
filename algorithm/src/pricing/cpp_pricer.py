...
NATIVE_DIR  = ROOT / "src" / "pricing" / "native"
...
def _collect_nonrequired_transition_edges(
    *,
    ...
    cur = depot
    for u, v in svc_seq:
        e = (cur, u) if cur < u else (u, cur)
        if cur != u and e in allowed and e not in seen:
            seen.add(e)
            used.append(e)
        cur = v
    e = (cur, depot) if cur < depot else (depot, cur)
    if cur != depot and e in allowed and e not in seen:
        used.append(e)
    return used
